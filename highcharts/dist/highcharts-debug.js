...
define("ucloud/highcharts/3.0.3/highcharts-debug", [ "ucloud/jquery/1.8.2/jquery-debug" ], function(require, exports, module) {
    ...
    globalAnimation, pathAnim, timeUnits, noop = function() {}, charts = [], PRODUCT = "Highcharts", VERSION = "3.0.2", // some constants for frequently used strings
 ...
 */
    function numberFormat(number, decimals, decPoint, thousandsSep) {
        ...
        n = number, c = decimals === -1 ? ((n || 0).toString().split(".")[1] || "").length : // preserve decimals
        isNaN(decimals = mathAbs(decimals)) ? 2 : decimals, d = decPoint === undefined ? lang.decimalPoint : decPoint, t = thousandsSep === undefined ? lang.thousandsSep : thousandsSep, s = n < 0 ? "-" : "", i = String(pInt(n = mathAbs(+n || 0).toFixed(c))), j = i.length > 3 ? i.length % 3 : 0;
 ...
 */
	 ...
	 */
        init: function(container, width, height, forExport) {
            var renderer = this, loc = location, boxWrapper, desc;
 ...
 */
	 ...
	 */
        addLabel: function() {
            var tick = this, axis = tick.axis, options = axis.options, chart = axis.chart, horiz = axis.horiz, categories = axis.categories, names = axis.series[0] && axis.series[0].names, pos = tick.pos, labelOptions = options.labels, str, tickPositions = axis.tickPositions, width = horiz && categories && !labelOptions.step && !labelOptions.staggerLines && !labelOptions.rotation && chart.plotWidth / tickPositions.length || !horiz && (chart.optionsMarginLeft || chart.plotWidth / 2), // #1580
 ...
 */
	 ...
	 */
        render: function(group) {
            var options = this.options, formatOption = options.format, // docs: added stackLabel.format option
 ...
 */
	 ...
	 */
        getSeriesExtremes: function() {
            var axis = this, chart = axis.chart, stacks = axis.stacks, posStack = [], negStack = [], stacksTouched = axis._stacksTouched = axis._stacksTouched + 1, type, i;
            ...
            each(axis.series, function(series) {
                if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
                    var seriesOptions = series.options, stacking, posPointStack, negPointStack, stackKey, stackOption, negKey, xData, yData, x, y, threshold = seriesOptions.threshold, yDataLength, activeYData = [], seriesDataMin, seriesDataMax, activeCounter = 0;
	 ...
	 */
        getOffset: function() {
            ...
            axisOffset = chart.axisOffset, clipOffset = chart.clipOffset, directionFactor = [ -1, 1, 1, -1 ][side], n;
	 ...
	 */
        refresh: function(point, mouseEvent) {
            ...
            if (crosshairsOptions) {
                ...
                // [x, y]
                var path, i = crosshairsOptions.length, attribs, axis, val;
                while (i--) {
 ...
 */
	 ...
	 */
        renderTitle: function() {
            var options = this.options, padding = this.padding, titleOptions = options.title, titleHeight = 0;
 ...
 */
	 ...
	 */
        redraw: function(animation) {
            var chart = this, axes = chart.axes, series = chart.series, pointer = chart.pointer, legend = chart.legend, redrawLegend = chart.isDirtyLegend, hasStackedSeries, isDirtyBox = chart.isDirtyBox, // todo: check if it has actually changed?
 ...
 */
	 ...
	 */
        update: function(options, redraw, animation) {
            var point = this, series = point.series, graphic = point.graphic, i, data = series.data, chart = series.chart;
            redraw = pick(redraw, true);
 ...
 */
	 ...
	 */
        setData: function(data, redraw) {
            ...
            // parallel arrays
            var xData = [], yData = [], zData = [], dataLength = data ? data.length : [], turboThreshold = options.turboThreshold || 1e3, pt, pointArrayMap = series.pointArrayMap, valueCount = pointArrayMap && pointArrayMap.length, hasToYData = !!series.toYData;
	 ...
	 */
        translate: function() {
            ...
            this.generatePoints();
            var series = this, options = series.options, stacking = options.stacking, xAxis = series.xAxis, categories = xAxis.categories, yAxis = series.yAxis, points = series.points, dataLength = points.length, hasModifyValue = !!series.modifyValue, isBottomSeries, allStackSeries, i, placeBetween = options.pointPlacement === "between", threshold = options.threshold;
            //nextSeriesDown;
            ...
            for (i = 0; i < dataLength; i++) {
                ...
                point.clientX = placeBetween ? xAxis.translate(xValue, 0, 0, 0, 1) : point.plotX;
                // #1514
	 ...
	 */
        clipNeg: function() {
            var options = this.options, chart = this.chart, renderer = chart.renderer, negativeColor = options.negativeColor, translatedThreshold, posAttr, negAttr, graph = this.graph, area = this.area, posClip = this.posClip, negClip = this.negClip, chartWidth = chart.chartWidth, chartHeight = chart.chartHeight, chartSizeMax = mathMax(chartWidth, chartHeight), above, below;
	 ...
	 */
        plotGroup: function(prop, name, visibility, zIndex, parent) {
            var group = this[prop], isNew = !group, chart = this.chart, xAxis = this.xAxis, yAxis = this.yAxis;
 ...
 */
	 ...
	 */
        getSegments: function() {
            var segments = [], segment = [], keys = [], xAxis = this.xAxis, yAxis = this.yAxis, stack = yAxis.stacks[this.stackKey], pointMap = {}, plotX, plotY, points = this.points, i, x;
	 ...
	 */
        drawGraph: function() {
            ...
            var series = this, areaPath = this.areaPath, options = this.options, negativeColor = options.negativeColor, props = [ [ "area", this.color, options.fillColor ] ];
            ...
            each(props, function(prop) {
                ...
                if (area) {
                    ...
                    series[areaKey] = series.chart.renderer.path(areaPath).attr({
                        fill: pick(prop[2], Color(prop[1]).setOpacity(options.fillOpacity || .75).get()),
                        zIndex: 0
                    }).add(series.group);
                }
            });
 ...
 */
	 ...
	 */
        getColumnMetrics: function() {
            var series = this, chart = series.chart, options = series.options, xAxis = this.xAxis, reversedXAxis = xAxis.reversed, stackKey, stackGroups = {}, columnIndex, columnCount = 0;
	 ...
	 */
        drawTracker: function() {
            var series = this, pointer = series.chart.pointer, cursor = series.options.cursor, css = cursor && {
                cursor: cursor
            }, onMouseOver = function(e) {
