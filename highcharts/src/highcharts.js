 ...
 */
	function attr(elem, prop, value) {
		var key, setAttribute = 'setAttribute',
		ret;
 ...
 */
	function numberFormat(number, decimals, decPoint, thousandsSep) {
		...
		n = number,
		c = decimals === - 1 ? ((n || 0).toString().split('.')[1] || '').length: // preserve decimals
 ...
 */
	function formatSingle(format, val) {
		...
		if (floatRegex.test(format)) { // float
			...
			val = numberFormat(
			val, decimals, lang.decimalPoint, format.indexOf(',') > - 1 ? lang.thousandsSep: '');
		} else {
 ...
 */
	/*jslint white: true*/
	timeUnits = hash(
	MILLISECOND, 1, SECOND, 1000, MINUTE, 60000, HOUR, 3600000, DAY, 24 * 3600000, WEEK, 7 * 24 * 3600000, MONTH, 31 * 24 * 3600000, YEAR, 31556952000);
	/*jslint white: false*/
	/**
 ...
 */
	 ...
	 */
		attr: function(hash, val) {
			var wrapper = this,
			key, value, result, i, child, element = wrapper.element,
				...
				for (key in hash) {
					...
					if (result !== false) {
						...
						if (shadows && /^(width|height|visibility|x|y|d|transform)$/.test(key)) {
							...
							while (i--) {
								attr(
								shadows[i], key, key === 'height' ? mathMax(value - (shadows[i].cutHeight || 0), 0) : value);
							}
						}
	...
	*/
	 ...
	 */
		symbolAttr: function(hash) {
			...
			wrapper.attr({
				...
				wrapper.x, wrapper.y, wrapper.width, wrapper.height, wrapper)
			});
		},
	 ...
	 */
		htmlUpdateTransform: function() {
			...
			if (elem.tagName === 'SPAN') {
				...
				var width, height, rotation = wrapper.rotation,
				baseline, radians = 0,
				costheta = 1,
				sintheta = 0,
				quad, textWidth = pInt(wrapper.textWidth),
	 ...
	 */
		getBBox: function() {
			...
			bBox = wrapper.bBox,
			renderer = wrapper.renderer,
			width, height, rotation = wrapper.rotation,
			element = wrapper.element,
 ...
 */
	 ...
	 */
		buildText: function(wrapper) {
			...
			lines = pick(wrapper.textStr, '').toString().replace(/<(b|strong)>/g, '<span style="font-weight:bold">').replace(/<(i|em)>/g, '<span style="font-style:italic">').replace(/<a/g, '<span').replace(/<\/(b|strong|i|em|a)>/g, '</span>').split(/<br.*?>/g),
				...
				each(spans, function(span) {
					if (span !== '' || spans.length === 1) {
						...
						if (!spanNo && lineNo) {
							...
							attr(
							tspan, 'dy', textLineHeight || renderer.fontMetrics(/px$/.test(tspan.style.fontSize) ? tspan.style.fontSize: textStyles.fontSize).h,
	 ...
	 */
		symbol: function(symbol, x, y, width, height, options) {
			...
			mathRound(x), mathRound(y), width, height, options),
	 ...
	 */
		color: function(color, elem, prop) {
			var renderer = this,
			colorObject, regexRgba = /^rgba/,
			gradName, gradAttr, gradients, gradientObject, stops, stopColor, stopOpacity, radialReference, n, id, key = [];
 ...
 */
	 ...
	 */
			attr: function(hash, val) {
				var wrapper = this,
				key, value, i, result, element = wrapper.element || {},
				elemStyle = element.style,
				nodeName = element.nodeName,
				renderer = wrapper.renderer,
				symbolName = wrapper.symbolName,
				hasSetSymbolSize, shadows = wrapper.shadows,
				skipAttr, attrSetters = wrapper.attrSetters,
				ret = wrapper;
	 ...
	 */
			color: function(color, elem, prop, wrapper) {
				...
				if (fillType) {
					...
					var stopColor, stopOpacity, gradient = color.linearGradient || color.radialGradient,
					x1, y1, x2, y2, opacity1, opacity2, color1, color2, fillAttr = '',
					stops = color.stops,
					firstStop, lastStop, colors = [],
					...
					if (prop === 'fill') {
						...
						if (fillType === 'gradient') {
							...
							cy = gradient.cy,
							radialReference = elem.radialReference,
							bBox, applyRadialGradient = function() {
								if (radialReference) {
 ...
 */
	 ...
	 */
		addLabel: function() {
			...
			width = (horiz && categories && ! labelOptions.step && ! labelOptions.staggerLines && ! labelOptions.rotation && chart.plotWidth / tickPositions.length) || (!horiz && (chart.optionsMarginLeft || chart.plotWidth / 2)),
			// #1580
			...
			if (!defined(label)) {
				...
				tick.label = defined(str) && labelOptions.enabled ? chart.renderer.text(
				str, 0, 0, labelOptions.useHTML).attr(attr)
	 ...
	 */
		getLabelSides: function() {
			...
			// assume getLabelSize has run at this point
			axis = this.axis,
			options = axis.options,
			labelOptions = options.labels,
			width = bBox.width,
	 ...
	 */
		getMarkPath: function(x, y, tickLength, tickWidth, horiz, renderer) {
			...
			M, x, y, L, x + (horiz ? 0: - tickLength), y + (horiz ? tickLength: 0)], tickWidth);
		},
	 ...
	 */
		render: function(index, old, opacity) {
			...
			x = xy.x,
			y = xy.y,
			reverseCrisp = ((horiz && x === axis.pos) || (!horiz && y === axis.pos + axis.len)) ? - 1: 1,
			// #1480
			staggerLines = axis.staggerLines;
 ...
 */
	 ...
	 */
		render: function(group) {
			...
			// docs: added stackLabel.format option
 ...
 */
	 ...
	 */
		setOptions: function(userOptions) {
			this.options = merge(
			this.defaultOptions, this.isXAxis ? {}: this.defaultYAxisOptions, [this.defaultTopAxisOptions, this.defaultRightAxisOptions, this.defaultBottomAxisOptions, this.defaultLeftAxisOptions][this.side], merge(
		...
		getSeriesExtremes: function() {
			...
			each(axis.series, function(series) {
				...
				if (series.visible || ! chart.options.chart.ignoreHiddenSeries) {
					...
					stacking, posPointStack, negPointStack, stackKey, stackOption, negKey, xData, yData, x, y, threshold = seriesOptions.threshold,
					...
					if (axis.isXAxis) {
						...
						var isNegative, pointStack, key, cropped = series.cropped,
	 ...
	 */
		translate: function(val, backwards, cvsCoord, old, handleLog, pointPlacementBetween) {
			...
			if (backwards) { // reverse translation
				...
				returnValue = sign * (val - localMin) * localA + cvsOffset + (sign * minPixelPadding) + (pointPlacementBetween ? localA * axis.pointRange / 2: 0);
			}
	 ...
	 */
		getPlotLinePath: function(value, lineWidth, old, force) {
			...
			axisLeft = axis.left,
			axisTop = axis.top,
			x1, y1, x2, y2, translatedValue = axis.translate(value, null, null, old),
			cHeight = (old && chart.oldChartHeight) || chart.chartHeight,
	 ...
	 */
		getPlotBandPath: function(from, to) {
			...
			if (path && toPath) {
				path.push(
				toPath[4], toPath[5], toPath[1], toPath[2]);
	 ...
	 */
		getLogTickPositions: function(interval, min, max, minor) {
			...
			if (interval >= 0.5) {
				...
				interval = pick(
				filteredTickIntervalOption, axis._minorAutoInterval, (realMax - realMin) * tickPixelIntervalOption / (totalPixelLength || 1));

				interval = normalizeTickInterval(
				interval, null, math.pow(10, mathFloor(math.log(interval) / math.LN10)));

				positions = map(axis.getLinearTickPositions(
				interval, realMin, realMax), log2lin);
	 ...
	 */
		getMinorTickPositions: function() {
			...
			if (axis.isLog) {
				...
				minorTickPositions = minorTickPositions.concat(
				getTimeTicks(
				normalizeTimeTickInterval(minorTickInterval), axis.min, axis.max, options.startOfWeek));
	 ...
	 */
		adjustForMinRange: function() {
			...
			min = axis.min,
			max = axis.max,
			zoomOffset, spaceAvailable = axis.dataMax - axis.dataMin >= axis.minRange,
	 ...
	 */
		setAxisTranslation: function(saveOld) {
			...
			ordinalCorrection, transA = axis.transA;
			...
			if (axis.isXAxis) {
				if (linkedParent) {
					...
					each(axis.series, function(series) {
						...
						minPointOffset, pointPlacement ? 0: seriesPointRange / 2);

						// Determine the total padding needed to the length of the axis to make room for the 
						// pointRange. If the series' pointPlacement is 'on', no padding is added.
						pointRangePadding = mathMax(
						pointRangePadding, pointPlacement === 'on' ? 0: seriesPointRange);
	 ...
	 */
		setTickPositions: function(secondPass) {
			...
			if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
				...
				tickIntervalOption, categories ? // for categoried axis, 1 is default, for linear axis use tickPix
			...
			if (!tickPositions) {
				if (isDatetimeAxis) {
					...
					normalizeTimeTickInterval(axis.tickInterval, options.units), axis.min, axis.max, options.startOfWeek, axis.ordinalPositions, axis.closestPointRange, true);
				} else if (isLog) {
	 ...
	 */
		setScale: function() {
			var axis = this,
			stacks = axis.stacks,
			type, i, isDirtyData, isDirtyAxisLength;
		...
		getOffset: function() {
			...
			if (hasData || axis.isLinked) {
				...
				each(tickPositions, function(pos) {
					...
					if (side === 0 || side === 2 || {
						...
						labelOffset = mathMax(
						ticks[pos].getLabelSize(), labelOffset);
					}
			...
			if (axisTitleOptions && axisTitleOptions.text && axisTitleOptions.enabled !== false) {
				if (!axis.axisTitle) {
					...
					axisTitleOptions.text, 0, 0, axisTitleOptions.useHTML).attr({
						zIndex: 7,
			...
			axisOffset[side], axis.axisTitleMargin + titleOffset + directionFactor * axis.offset);
	 ...
	 */
		getLinePath: function(lineWidth) {
			...
			M, horiz ? this.left: lineLeft, horiz ? lineTop: this.top, L, horiz ? chart.chartWidth - this.right: lineLeft, horiz ? lineTop: chart.chartHeight - this.bottom], lineWidth);
		},
	 ...
	 */
		render: function() {
			...
			linePath, hasRendered = chart.hasRendered,
 ...
 */
	 ...
	 */
		refresh: function(point, mouseEvent) {
			...
			if (crosshairsOptions) {
				...
				var path, i = crosshairsOptions.length,
				attribs, axis, val;
	 ...
	 */
		updatePosition: function(point) {
			...
			this, label.width, label.height, point);

			// do the move
			this.move(
			mathRound(pos.x), mathRound(pos.y), point.plotX + chart.plotLeft, point.plotY + chart.plotTop);
		}
	};
	/**
 ...
 */
	 ...
	 */
		runPointActions: function(e) {
			...
			point, points, hoverPoint = chart.hoverPoint,
			hoverSeries = chart.hoverSeries,
			i, j, distance = chart.chartWidth,
	 ...
	 */
		drag: function(e) {
			...
			plotWidth = chart.plotWidth,
			plotHeight = chart.plotHeight,
			clickedInside, size, mouseDownX = this.mouseDownX,
			...
			if (this.hasDragged > 10) {
				...
				if (chart.hasCartesianSeries && (this.zoomX || this.zoomY) && clickedInside) {
					if (!this.selectionMarker) {
						...
						plotLeft, plotTop, zoomHor ? 1: plotWidth, zoomVert ? 1: plotHeight, 0).attr({
 ...
 */
	 ...
	 */
		positionItem: function(item) {
			...
			if (item.legendGroup) {
				...
				ltr ? itemX: legend.legendWidth - itemX - 2 * symbolPadding - 4, itemY);
			}
	 ...
	 */
		renderItem: function(item) {
			...
			if (!li) { // generate it once, later move it
				...
				options.labelFormat ? format(options.labelFormat, item) : options.labelFormatter.call(item), ltr ? symbolWidth + symbolPadding: - symbolPadding, legend.baseline, useHTML).css(merge(item.visible ? itemStyle: itemHiddenStyle)) // merge to prevent modifying original (#1021)
				.attr({
			...
			horizontal ? legend.itemX - initialItemX: itemWidth, legend.offsetWidth);
		},
		...
		render: function() {
			...
			if (legendBorderWidth || legendBackgroundColor) {
				...
				if (!box) {
					...
					0, 0, legendWidth, legendHeight, options.borderRadius, legendBorderWidth || 0).attr({
	 ...
	 */
		handleOverflow: function(legendHeight) {
			...
			chart = this.chart,
			renderer = chart.renderer,
			pageCount, options = this.options,
			optionsY = options.y,
 ...
 */
	 ...
	 */
		redraw: function(animation) {
			...
			hasStackedSeries, isDirtyBox = chart.isDirtyBox,
			// todo: check if it has actually changed?
			seriesLength = series.length,
			i = seriesLength,
			serie, renderer = chart.renderer,
		...
		setTitle: function(titleOptions, subtitleOptions) {
			...
			each([['title', titleOptions, chartTitleOptions], ['subtitle', subtitleOptions, chartSubtitleOptions]], function(arr) {
				...
				if (chartTitleOptions && chartTitleOptions.text && ! title) {
					...
					chartTitleOptions.text, 0, 0, chartTitleOptions.useHTML).attr({
						align: chartTitleOptions.align,
						'class': PREFIX + name,
						zIndex: chartTitleOptions.zIndex || 4
					}).css(chartTitleOptions.style).add().align(chartTitleOptions, false, 'spacingBox');
				}
			});
	 ...
	 */
		getContainer: function() {
			var chart = this,
			container, optionsChart = chart.options.chart,
			chartWidth, chartHeight, renderTo, indexAttrName = 'data-highcharts-chart',
			oldChartIndex, containerId;
	 ...
	 */
		getMargins: function() {
			...
			axisOffset, legend = chart.legend,
			...
			if (legend.display && ! legendOptions.floating) {
				if (align === 'right') { // horizontal alignment handled first
					if (!defined(optionsMarginRight)) {
						...
						chart.marginRight, legend.legendWidth - legendX + legendMargin + spacingRight);
					}
				} else if (align === 'left') {
					if (!defined(optionsMarginLeft)) {
						chart.plotLeft = mathMax(
						chart.plotLeft, legend.legendWidth + legendX + legendMargin + spacingLeft);
					}

				} else if (verticalAlign === 'top') {
					if (!defined(optionsMarginTop)) {
						chart.plotTop = mathMax(
						chart.plotTop, legend.legendHeight + legendY + legendMargin + spacingTop);
					}

				} else if (verticalAlign === 'bottom') {
					if (!defined(optionsMarginBottom)) {
						chart.marginBottom = mathMax(
						chart.marginBottom, legend.legendHeight - legendY + legendMargin + spacingBottom);
					}
				}
			}
	 ...
	 */
		setSize: function(width, height, animation) {
			var chart = this,
			chartWidth, chartHeight, fireEndResize;
	 ...
	 */
		render: function() {
			...
			if (credits.enabled && ! chart.credits) {
				...
				credits.text, 0, 0).on('click', function() {
					if (creditsHref) {
	 ...
	 */
		setState: function(state) {
			...
			if (point.graphic) {
				...
				pointAttr[state], radius ? { // new symbol attributes (#507, #612)
					x: plotX - radius,
					y: plotY - radius,
					width: 2 * radius,
					height: 2 * radius
				...
				if (state && markerStateOptions) {
					...
					if (!stateMarkerGraphic) {
						...
						newSymbol, plotX - radius, plotY - radius, 2 * radius, 2 * radius).attr(pointAttr[state]).add(series.markerGroup);
 ...
 */
	...
	Series.prototype = {
		...
		init: function(chart, options) {
			var series = this,
			eventType, events, linkedTo, chartSeries = chart.series;
	 ...
	 */
		setOptions: function(itemOptions) {
			...
			options = merge(
			typeOptions, plotOptions.series, itemOptions);
	 ...
	 */
		drawLegendSymbol: function(legend) {
			...
			radius, legendOptions = legend.options,
			legendSymbol, symbolWidth = legendOptions.symbolWidth,
			...
			if (options.lineWidth) {
				...
				M, 0, baseline - 4, L, symbolWidth, baseline - 4]).attr(attr).add(legendItemGroup);
			}
			...
			if (markerOptions && markerOptions.enabled) {
				...
				this.symbol, (symbolWidth / 2) - radius, baseline - 4 - radius, 2 * radius, 2 * radius).add(legendItemGroup);
			}
		},
	 ...
	 */
		translate: function() {
			...
			hasModifyValue = !! series.modifyValue,
			isBottomSeries, allStackSeries, i, placeBetween = options.pointPlacement === 'between',
	 ...
	 */
		setTooltipPoints: function(renew) {
			...
			// tooltipLen and tooltipPosName used in polar
			point,
			i,
			...
			for (i = 0; i < pointsLength; i++) {
				...
				high = points[i + 1] ? mathMax(0, mathFloor((point.clientX + (points[i + 1] ? points[i + 1].clientX: axisLength)) / 2)) : axisLength;
	 ...
	 */
		animate: function(init) {
			...
			chart = series.chart,
			renderer = chart.renderer,
			clipRect, markerClipRect, animation = series.options.animation,
			clipBox = chart.clipBox,
			inverted = chart.inverted,
			sharedClipKey;
	 ...
	 */
		drawPoints: function() {
			...
			pointMarkerOptions, enabled, isInside, markerGroup = series.markerGroup;
				...
				while (i--) {
					...
					if (enabled && plotY !== UNDEFINED && ! isNaN(plotY) && point.y !== null) {
						...
						if (graphic) { // update
							...
							symbol, plotX - radius, plotY - radius, 2 * radius, 2 * radius).attr(pointAttr).add(markerGroup);
						}
		...
		getAttribs: function() {
			...
			while (i--) {
				...
				if (hasPointSpecificOptions) {
					...
					stateOptions[HOVER_STATE], seriesPointAttr[HOVER_STATE], pointAttr[NORMAL_STATE]);

					// inherit from point normal and series hover
					pointAttr[SELECT_STATE] = series.convertAttribs(
					stateOptions[SELECT_STATE], seriesPointAttr[SELECT_STATE], pointAttr[NORMAL_STATE]);
	 ...
	 */
		drawDataLabels: function() {
			...
			points = series.points,
			pointOptions, generalOptions, str, dataLabelsGroup;
			...
			if (options.enabled || series._hasPointLabels) {
				...
				each(points, function(point) {
					...
					labelConfig, attr, name, rotation, connector = point.connector,
					isNew = true;
	 ...
	 */
		alignDataLabel: function(point, dataLabel, options, alignTo, isNew) {
			...
			dataLabel.attr({
				...
				chart.isInsidePlot(plotX, plotY, inverted) ? (chart.renderer.isSVG ? 'inherit': VISIBLE) : HIDDEN
			});
	 ...
	 */
		clipNeg: function() {
			...
			renderer = chart.renderer,
			negativeColor = options.negativeColor,
			translatedThreshold, posAttr, negAttr, graph = this.graph,
			area = this.area,
	 ...
	 */
		render: function() {
			...
			doAnimation = animation && !! series.animate && chart.renderer.isSVG,
			// this animation doesn't work in IE8 quirks when the group div is hidden,
 ...
 */
	 ...
	 */
		closeSegment: function(path, segment) {
			...
			path.push(
			L, segment[segment.length - 1].plotX, translatedThreshold, L, segment[0].plotX, translatedThreshold);
		},
			...
			each(props, function(prop) {
				...
				if (area) { // update
					...
					series[areaKey] = series.chart.renderer.path(areaPath).attr({
						fill: pick(
						prop[2], Color(prop[1]).setOpacity(options.fillOpacity || 0.75).get()),
						zIndex: 0 // #1069
					}).add(series.group);
				}
			});
	 ...
	 */
		drawLegendSymbol: function(legend, item) {
			...
			0, legend.baseline - 11, legend.options.symbolWidth, 12, 2).attr({
				zIndex: 3
			}).add(item.legendGroup);
 ...
 */
	 ...
	 */
		getColumnMetrics: function() {
			...
			mathAbs(xAxis.transA) * (xAxis.ordinalSlope || options.pointRange || xAxis.closestPointRange || 1), xAxis.len // #1535
			),
	 ...
	 */
		translate: function() {
			...
			each(series.points, function(point) {
				var plotY = mathMin(mathMax( - 999, point.plotY), yAxis.len + 999),
				// Don't draw too far outside plot area (#1303)
	 ...
	 */
		alignDataLabel: function(point, dataLabel, options, alignTo, isNew) {
			...
			options.verticalAlign, inverted || inside ? 'middle': below ? 'top': 'bottom');
 ...
 */
	 ...
	 */
		translate: function(positions) {
			...
			start, end, angle, startAngleRad = series.startAngleRad = mathPI / 180 * ((options.startAngle || 0) % 360 - 90),
			points = series.points,
			circ = 2 * mathPI,
			fraction,
			radiusX,
		...
		drawPoints: function() {
			...
			each(series.points, function(point) {
				...
				if (graphic) {
					...
					point.graphic = graphic = renderer.arc(shapeArgs).setRadialReference(series.center).attr(
					point.pointAttr[point.selected ? SELECT_STATE: NORMAL_STATE]).attr({
						'stroke-linejoin': 'round'
	 ...
	 */
		verifyDataLabelOverflow: function(overflow) {
			...
			if (centerOption[1] !== null) { // Fixed center
				...
				newSize = mathMax(
				mathMin(
				newSize, center[2] - overflow[0] - overflow[2] // vertical overflow
				), minSize);
	...
	// global variables
	module.exports = extend(Highcharts, {
